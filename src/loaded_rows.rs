...
#[derive(Clone)]
pub enum RowState<T: Clone + Sync + Send> {
...
impl<T: Clone + Sync + Send> std::fmt::Debug for RowState<T> {
...
pub struct LoadedRows<T: Clone + Sync + Send> {
    rows: Vec<RowState<T>>,
}

impl<T: Clone + Sync + Send> LoadedRows<T> {
    pub fn new() -> Self {
        Self { rows: vec![] }
    }
...
impl<T: Clone + Sync + Send> Index<Range<usize>> for LoadedRows<T> {
...
impl<T: Clone + Sync + Send> Index<usize> for LoadedRows<T> {
    type Output = RowState<T>;
